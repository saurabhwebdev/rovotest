...
interface GateGuardAuditTrailProps {
  showFullHistoryByDefault?: boolean;
}

export default function GateGuardAuditTrail({ showFullHistoryByDefault = false }: GateGuardAuditTrailProps) {
  ...
  const getActionBadgeColor = (action: string) => {
    switch (action) {
      ...
      case 'ASSIGNED_TO_WEIGHBRIDGE':
        return 'bg-blue-100 text-blue-800 dark:bg-blue-800 dark:text-blue-100';
      case 'ASSIGNED_TO_PARKING':
            ...
            {auditEntries.map((entry) => (
              <tr key={entry.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                ...
                <td className="px-4 py-3 text-sm text-gray-500 dark:text-gray-300">
                  {entry.details.status && <div>Status: {entry.details.status}</div>}
                  {entry.details.location && <div>Location: {entry.details.location}</div>}
                  {entry.details.notes && <div>Notes: {entry.details.notes}</div>}
                  {entry.details.weighbridgeName && <div>Weighbridge: {entry.details.weighbridgeName}</div>}
                  {entry.details.failedChecks && entry.details.failedChecks.length > 0 && (
                    <div>
                      Failed Checks: {entry.details.failedChecks.join(', ')}
                    </div>
                  )}
